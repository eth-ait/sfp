...
# Implements the PolyRL algorithm, taken from https://github.com/h-aboutalebi/SparseBaseline
# It is implemented based on psudo code of algorithm
class PolyRL():

    def __init__(self, gamma, env, max_action=1., min_action=-1., actor_target_function=None):

        self.nb_actions = env.action_space.shape[-1]
        if isinstance(env.observation_space, gym.spaces.dict.Dict):
            self.nb_observations = env.observation_space['observation'].shape[0] + env.observation_space['desired_goal'].shape[0]
        else:
            self.nb_observations = env.observation_space.shape[0]

        if 'opper' in env._env_name:
            self.lambda_, self.betta, self.sigma_squared = 0.35, 0.01, 0.017
        elif 'heetah' in env._env_name:
            self.lambda_, self.betta, self.sigma_squared = 0.35, 0.05, 0.00007
        else:
            self.lambda_, self.betta, self.sigma_squared = 0.35, 0.01, 0.017
    ...
    def select_action(self, state, previous_action, step_number):
        ...
        # if (step_number % 1000 == 0):
            # print("Percentage of target policy exploration call in PolyRl: {}".format(self.percentage_exploration))
